		...
		function traverseNestedData(_node) {
			if(childrenAccessor(_node)) { // if there are children of this data (i.e. planets, moons)
				...
				for (var x = 0; x<totalChildren;x++) {
					...
					if(+childrenAccessor(_node)[x].semi_major > maxDistance && childrenAccessor(_node)[x].semi_major != "NA"){
						maxDistance = +childrenAccessor(_node)[x].semi_major;
					}
					if(+childrenAccessor(_node)[x].semi_major < minDistance){
						minDistance = +childrenAccessor(_node)[x].semi_major;
					}
